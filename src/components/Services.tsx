...
const services = [{
  icon: Users, title: "Staff & Payroll Outsourcing",
  ...
  features: ["Recruitment & Onboarding", "Payroll Management", "Compliance Handling", "HR Administration"], popular: true
}, {
  icon: HardHat, title: "Labour Supply",
  ...
  icon: Building2, title: "Corporate Housekeeping",
  ...
  icon: Sparkles, title: "Washroom Hygiene Management",
  ...
  icon: Building2, title: "Floor Care Services",
  ...
  icon: Wrench, title: "MEP & Technical Services",
  ...
  icon: ShieldCheck, title: "Professional Security Services",
  ...
  icon: Coffee, title: "Pantry & Cafeteria Services",
...
const Services = () => {
  ...
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === "Escape" && selectedService) setSelectedService(null);
    };
  ...
  return (
    <>
      <section id="services" className="section-padding bg-gradient-subtle" ref={ref}>
        <div className="section-container">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center max-w-2xl mx-auto mb-16">
            <motion.span initial={{ opacity: 0, scale: 0.9 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.5, delay: 0.1 }} className="inline-flex items-center gap-2 px-4 py-2 mb-4 text-sm font-semibold rounded-full bg-accent/10 text-accent border border-accent/20">
              <Sparkles className="w-4 h-4" />
              What We Do
            </motion.span>
            <h2 className="heading-lg text-primary mb-5">
              Comprehensive{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-purple-dark">Facility Solutions</span>
            </h2>
            <p className="text-body">
              From staffing to security, we provide end-to-end workforce and facility management services tailored to your business needs.
            </p>
          </motion.div>

          {/* Services Grid - Added md:grid-cols-3 */}
          <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
            {services.map((service, index) => (
              <motion.div key={service.title} variants={itemVariants} onClick={() => setSelectedService(service)} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setSelectedService(service); } }} tabIndex={0} role="button" aria-label={`View details for ${service.title}`} className="group relative bg-card rounded-xl md:rounded-2xl p-4 md:p-6 cursor-pointer focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 overflow-hidden">
                ...
                <div className="relative z-10">
                  {service.popular && <div className="absolute -top-1 -right-1"><span className="inline-block px-1.5 md:px-2 py-0.5 text-[8px] md:text-[10px] font-bold rounded-full bg-accent text-accent-foreground shadow-lg">POPULAR</span></div>}
                  ...
                  <h3 className="font-display font-bold text-primary mb-2 md:mb-3 text-sm md:text-lg leading-tight group-hover:text-accent transition-colors duration-300">{service.title}</h3>
                  <p className="hidden md:block text-muted-foreground mb-4 leading-relaxed text-sm line-clamp-2">{service.description}</p>
                  <div className="hidden md:flex flex-wrap gap-1.5 mb-4">
                    {service.features.slice(0, 2).map(feature => <span key={feature} className="inline-block px-2.5 py-1 text-[11px] font-medium rounded-lg bg-accent/10 text-accent">{feature}</span>)}
                    {service.features.length > 2 && <span className="inline-block px-2.5 py-1 text-[11px] font-medium rounded-lg bg-muted text-muted-foreground">+{service.features.length - 2} more</span>}
                  </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
              <DialogHeader>
                <div className="flex items-center gap-4 mb-3">
                  ...
                  <div>
                    <DialogTitle className="text-xl font-display font-bold text-primary">{selectedService.title}</DialogTitle>
                    {selectedService.popular && <span className="inline-block mt-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-accent text-accent-foreground">MOST POPULAR</span>}
                  </div>
                </div>
              </DialogHeader>
              <div className="space-y-6">
                ...
                <div>
                  <h4 className="font-display font-semibold text-primary mb-4 text-sm flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-accent" /> What's Included:
                  </h4>
                  <ul className="grid grid-cols-2 gap-2">
                    {selectedService.features.map(feature => <li key={feature} className="flex items-center gap-2 text-sm text-muted-foreground"><span className="w-1.5 h-1.5 rounded-full bg-accent flex-shrink-0" />{feature}</li>)}
                  </ul>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <Button onClick={() => handleWhatsApp(selectedService.title)} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/20" size="lg">Enquire on WhatsApp</Button>
                  <Button onClick={() => handleRequestQuote(selectedService.title)} variant="accentOutline" className="flex-1" size="lg">Request a Quote</Button>
                </div>
              </div>
            </motion.div>
          )}
